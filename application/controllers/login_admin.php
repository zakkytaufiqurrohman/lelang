...
class Login_admin extends ci_controller{
	...
	public function index(){
		...
		$sql = $this->db->query("SELECT * FROM tbl_pelelang");
		foreach ($sql->result() as $row) {
					
		
				if($username == $row->username and $row->status == ''){
					$data['status'] = '0';
        ...
        $this->form_validation->set_rules('password', 'password', 'trim|required|xss_clean|callback_check_database');// disini terdapat callback : callback_check_database. digunakan untuk memanggil function check_database() dibawah.
        		...
        		if($this->form_validation->run() == FALSE){
        			...
        			redirect('index.php/web_admin?gagal=1','refresh');

				  }else{
				        			redirect('index.php/web_admin','refresh');
				        		}
					}
	...
	public function check_database(){
        ...
        $this->load->model('model_login');                 
		$result=$this->model_login->admin($username,$password);
